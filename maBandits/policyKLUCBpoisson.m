classdef policyKLUCBpoisson < Policy
    ...
    properties
        ...
        bound = 1  % if bounded is not equal to 1, then it is assumed that 
                   % the rewards are actually bounded Poisson
                   % variables that have been normalized to [0,1] (i.e.,
                   % divided by bound)        
    end
    
    methods
        function self = policyKLUCBpoisson(c, variant, bound)
            if (nargin == 1)
                self.c = c;
            end            
            if (nargin >= 2)
