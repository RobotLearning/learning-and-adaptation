...
ker = @(x1, x2) sigma_s2 * kernel(x1,x2,l,type) + (x1==x2) * var_noise;
...
if exist('mvnrnd')
    ...
    for i = 1:trials
        fun(:,i) = mu + chol(Sigma)*randn(meshsize,1);
    end
    fun = fun';
end
...
for i = 1:trials
    ...
    for t = 1:horizon
        ...
        % sample x
        [ind,x_new,Kinv,~] = gpo(rewards(i,1:t-1),x(1:t-1),Kinv,ker,mesh,acq);
        x(t) = x_new;
        % calculate the cost and add noise
        rewards(i,t) = fun(i,ind);
...
for i = 1:trials
    ...
    for t = 1:horizon
        ...
        % sample x
        [ind,x_new,Kinv,s2max] = gpo(rewards(i,1:t-1),x(1:t-1),Kinv,ker,mesh,acq);
        x(t) = x_new;
        % calculate the cost and add noise
        rewards(i,t) = fun(i,ind);
        % regret for not picking the optimum
